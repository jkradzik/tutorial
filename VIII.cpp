...
void VIII(void){
	...
	//ROBIE CUDA WIADNKI ZEBY STWORZYC DYNAMICZNA MACIERZ DWUWYMIAROWA
	int **macierzb = new int *[wier2]; //tablica wskaŸników <-- pe³ne szaleñstwo
	...
	kol3=kol2;
	//ROBIE CUDA WIADNKI ZEBY STWORZYC DYNAMICZNA MACIERZ DWUWYMIAROWA
	int **macierzc = new int *[wier3]; //tablica wskaŸników <-- pe³ne szaleñstwo
	for ( int i=0; i<wier3;i++)
	macierzc[i] = new int [kol3]; //alokacja pamieci	
	
	/* ================iloczyn A I B==========PE£NE SZALEÑSTWO xD=====*/
